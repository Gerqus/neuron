...
class Plotter {
    ...
    plotPoint(serie, lineIndex) {
        ...
        const valueLabel = utils_1.roundWithPrecision(value, 3).toString().padEnd(7, ' ');
        let formattedLabel;
    ...
    drawLabelsAndStatsLine() {
        this.series.forEach(serie => {
            process.stdout.cursorTo(serie.offset);
            process.stdout.write(chalk.rgb(...serie.color)(`${serie.label} <${utils_1.roundWithPrecision(serie.minValue, 3)};${utils_1.roundWithPrecision(serie.maxValue, 3)}>`));
        });
        process.stdout.write('\n');
    }
